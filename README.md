...
사내 규정 문서(PDF / PPTX / MD)를 인덱싱하고, 자연어로 질문하면 관련 규정을 찾아 답변하는 로컬 RAG 챗봇입니다.
매뉴얼, 계약서, 정책 문서 등 **한국어 문서 기반 QA가 필요한 모든 용도**에 그대로 활용할 수 있습니다.

- 외부 API 없이 완전 로컬 동작 (인터넷 불필요)
- 규정에 없는 내용은 LLM 호출 없이 즉시 "확인 불가" 반환 — 할루시네이션 방지
- 답변마다 근거 문서 출처(파일명, 페이지/섹션) 함께 표시
